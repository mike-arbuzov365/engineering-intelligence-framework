...
| Installable package (`eifctl`) | **Built, Ratified (D-05/D-08), not published** | `pip install .` gives 7 tested subcommands (`init`, `doctor`, `search`, `render`, `privacy-scan`, `validate`, `version`) without requiring a framework checkout. The installed-wheel suite currently passes 49/49 locally; its Windows/Ubuntu Python 3.11/3.12 matrix is part of the manual release gate under D-14, not routine PR CI. See [`docs/product/claims-evidence.md`](docs/product/claims-evidence.md). |
