...
| Installable package (`eifctl`) | **Built, Ratified (D-05/D-08), not published** | `pip install .` gives 7 tested subcommands (`init`, `doctor`, `search`, `render`, `privacy-scan`, `validate`, `version`) without requiring a framework checkout. The expanded installed-wheel suite passes 57/57 locally, including machine-readable integration states and injected upgrade rollback; the updated Windows/Ubuntu Python 3.11/3.12 matrix remains a manual release gate under D-14, not routine PR CI. See [`docs/product/claims-evidence.md`](docs/product/claims-evidence.md). |
...
| Optional integration contract | **Behavioral adapters built (experimental)** | RTK has version/argv/grep/diff canaries, a command registry and content-free local telemetry; Graphify has version/query/path/explain canaries, commit-derived freshness, local raw-artifact policy and a semantic cost gate. Both remain optional and degrade to core-safe fallbacks. Focused evidence is provider/version bounded, not a general performance claim; see [`integrations/README.md`](integrations/README.md). Vendor-docs remains declaration-only. |
...
| Benchmark | **First bounded pilot published** | 6/6 attempts succeeded across three synthetic tasks and two modes with one model, n=1 per cell. This proves the harness/pilot, not an efficiency advantage; see [`docs/benchmarks/README.md`](docs/benchmarks/README.md) |
