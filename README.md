...
| Bootstrap script (`eif_init.py`) | **Experimental** | [`scripts/eif_init.py`](scripts/eif_init.py) - initializes a **self-contained, separately-runnable** instance (real framework-ref provenance, a pinned `.eif/runtime/` bundle, a CLAUDE.md entrypoint), **non-destructively** (conflict guard, `--force` backup, `--dry-run`, CLAUDE.md marker merge). Does not ratify a CLI name/packaging (D-05/D-08 open) |
...
| Knowledge index / lifecycle-aware retrieval | **Available (experimental)** | [`scripts/eif_generate_index.py`](scripts/eif_generate_index.py), [`scripts/eif_search_knowledge.py`](scripts/eif_search_knowledge.py) - offline keyword search that returns only eligible statuses by default (excludes rejected/superseded), reports malformed artifacts distinctly, and handles Unicode/Ukrainian queries. No embeddings; not tested at scale |
