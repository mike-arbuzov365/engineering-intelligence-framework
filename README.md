...
| Installable package (`eifctl`) | **Built, Ratified (D-05/D-08)** | `pip install .` (not yet published to PyPI - see `pyproject.toml`) gives a real `eifctl` console command with 7 subcommands (`init`, `doctor`, `search`, `render`, `privacy-scan`, `validate`, `version`), each a thin wrapper over the exact same tested `scripts/eif_*.py` functions (byte-for-byte synced - see [`scripts/sync_package_sources.py`](scripts/sync_package_sources.py)), with schemas/locales/templates loaded via `importlib.resources` - no framework git checkout needed after install. `scripts/tests/test_package_build.py` builds a real wheel, installs it into a clean venv (path containing a space and non-ASCII text, confirming Unicode/space-safety), and runs every subcommand end-to-end, including a project path also containing a space and non-ASCII text, offline, from a cwd that is neither the framework checkout nor the project - 23/23 checks passing. The CI matrix (`.github/workflows/ci.yml`'s `package-build` job, Windows + Ubuntu x Python 3.11/3.12) ran the same suite and passed all 4 combinations, confirmed across two separate runs - see `core/policies/decisions.md`'s D-05/D-08 entry for the full evidence. The original standalone-script form (`python scripts/eif_init.py ...`) keeps working unchanged. |
| Bootstrap script (`eif_init.py`) | **Experimental** | [`scripts/eif_init.py`](scripts/eif_init.py) - initializes a **separately-runnable** instance: real dirty-checked framework provenance, a hashed `.eif/runtime/` source bundle, a CLAUDE.md entrypoint via a marker-safe merge (refuses to write, rather than corrupt, malformed `BEGIN`/`END` markers). Every managed artifact - config (when written), runtime bundle, lock, entrypoint, `.gitignore`, and the knowledge index (when `knowledge.managed: true`) - commits as one ordered transaction, with proven rollback of every already-committed stage if a later one fails - and a mid-copy crash that leaves a genuinely partial `.next` is cleaned up too (each `.next` is registered for cleanup before its first byte is written). Mode (init / routine upgrade / explicit `--force` reconfigure) is decided by what's already on disk: a routine upgrade derives project/adapter/locale/migration-status from the existing config and lock instead of CLI defaults, and never touches `.eif/config.yaml`. An existing but broken `.eif/config.yaml` (invalid YAML, not a mapping, or schema-invalid) stops before any write rather than being treated as absent. `knowledge.root`/`knowledge.index_path` are validated by a shared path-policy helper (`scripts/eif_paths.py`) that rejects absolute paths, Windows drive/UNC paths, `..` traversal, and any value resolving outside the instance. **Adoption-hardening**: a preflight (`scripts/eif_preflight.py`) detects pre-existing entrypoint governance before any write and stops (structured OK/WARN/STOP, shown identically by `--dry-run`) rather than silently appending a competing authority block - keyed off the resolved, persisted adoption basis, not just the current run's CLI flags: a run with no adoption decision on record STOPs uniformly regardless of mode (init/upgrade/reconfigure), while a persisted `coexist` decision authorizes safe continuation on later flagless runs (it prevents a fall-back to implicit greenfield, it does not itself STOP); an explicit user-owned `adoption.mode: coexist` generates a block that defers to existing project rules instead of claiming sole authority; an existing knowledge-index file without EIF's own ownership marker is never overwritten; `lock.instance.migration_status` (historical origin) is derived from a dedicated read-only **repository-origin** check (any project-owned file outside `.git/`/`.eif/`), separate from the governance preflight - so an existing code repo with **no `CLAUDE.md`** is recorded `adopted`, not `greenfield`, an unreadable origin fails closed, and the recorded origin can never contradict `adoption.mode`. See [`docs/architecture/instance-contract.md`](docs/architecture/instance-contract.md). Does not ratify a CLI name/packaging (D-05/D-08 open) |
