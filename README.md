...
| Bootstrap script (`eif_init.py`) | **Experimental** | [`scripts/eif_init.py`](scripts/eif_init.py) - initializes a **separately-runnable** instance: real dirty-checked framework provenance, a hashed `.eif/runtime/` source bundle (transactionally staged and swapped, with proven upgrade-failure rollback), a CLAUDE.md entrypoint, **non-destructively** (routine upgrade never touches `.eif/config.yaml`; `--force` backs it up first). See [`docs/architecture/instance-contract.md`](docs/architecture/instance-contract.md). Does not ratify a CLI name/packaging (D-05/D-08 open) |
