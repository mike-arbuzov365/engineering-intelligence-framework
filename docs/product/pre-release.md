...
**Now:** the same two commands, and the walkthrough they run.

**Why nothing on the page became false.** The command shown is `pip
install .`, which is what you run from a clone, and it works. The page
never printed `pip install engineering-intelligence-framework`, which is
the one form that would fail for a reader today, and an end-to-end test
...
5. **Manual:** set `EIF_REPOSITORY_URL` for the site build, which empties
   the "repository link is added at publication" line and points the
   closing screen's Source row at the real repository.
6. Re-read this file. Any line that stopped being true is a line to change,
   not a line to leave.
