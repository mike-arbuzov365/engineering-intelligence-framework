...
# Checks whose subject the managed-state refresh regenerates in the same
# transaction that replaces the runtime bundle. `eifctl upgrade` runs this
# script against the *pinned* runtime before replacing it, so that it refuses
# to upgrade on top of a bundle it cannot attest to. A drifted knowledge index
# is not a bundle it cannot attest to: eif_init.py rewrites the index and the
# lock hash that records it together, so failing the pre-check here blocked the
# one command that repairs the drift, and the remediation text pointed at
# eif_init.py - which is exactly what the blocked command wraps. Observed
# 2026-07-29 on a connected project after a hand-regenerated index.
# These are deferred, never skipped: --pre-upgrade only applies to the run
# *before* the refresh, and the post-upgrade doctor run checks every one of
# them again against the state the refresh just wrote.
DEFERRABLE_BEFORE_UPGRADE = frozenset({
    "knowledge index drift (ownership marker, hash)",
    "config/generated-block drift (adoption.mode, knowledge paths)",
})


class Report:
    def __init__(self, deferred: frozenset[str] = frozenset()):
    ...
    def print(self) -> bool:
        ...
        for title, problems in self.sections:
            if problems and title in self._deferred:
                print(f"note {title} - regenerated by the refresh; re-checked after it")
                for p in problems:
                    print(f"  - {p}")
            elif problems:
                ok = False
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Report drift that the managed-state refresh regenerates as a note "
            "instead of a failure. For the pre-flight run inside `eifctl "
            "upgrade` only - the post-upgrade run checks everything."
        ),
    )
    ...
    report = Report(DEFERRABLE_BEFORE_UPGRADE if args.pre_upgrade else frozenset())
