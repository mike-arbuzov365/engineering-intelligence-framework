...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Prior private-pilot finding: the generated block must not tell the
        # agent to read/search a managed index EIF never created here.
        entry2b = (inst2b / "CLAUDE.md").read_text(encoding="utf-8")
        results.append(check("2b. unmanaged knowledge: block does not instruct reading the non-existent managed index",
                             "Read `docs/knowledge/index.md` for what's already known" not in entry2b, entry2b))
        results.append(check("2b. unmanaged knowledge: block says knowledge is not managed by EIF",
                             "not managed by EIF" in entry2b, entry2b))
        results.append(check("2b. unmanaged knowledge: block omits the knowledge-root frontmatter validate command",
