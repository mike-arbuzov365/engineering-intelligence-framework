...
GITATTRIBUTES_MARKER = "# EIF:BEGIN gitattributes"
GITATTRIBUTES_END = "# EIF:END gitattributes"


def gitattributes_pattern(path: str) -> str:
    """A .gitattributes pattern for one instance-relative path. Whitespace
    has to be quoted; git parses the first unquoted space as the end of the
    pattern."""
    normalized = path.replace("\\", "/")
    return f'"{normalized}"' if any(c.isspace() for c in normalized) else normalized


def render_gitattributes_block(
    entrypoint: str,
    ...
    if knowledge_index_path:
        paths.append(knowledge_index_path)
    if workspace_content_root:
        # A private workspace's own artifacts are hashed into every connected
        # project's workspace lock. Left unpinned, the same artifact resolves
        # to different bytes on a checkout that converts newlines, so the
        # fleet would restage every project purely because of where it was
        # cloned.
        paths.append(f"{workspace_content_root.rstrip('/')}/**")
    lines = [
        f"{GITATTRIBUTES_MARKER} - managed by scripts/eif_init.py, do not hand-edit this block"
    ]
    seen: set[str] = set()
    for path in paths:
        pattern = gitattributes_pattern(path)
        if pattern in seen:
            continue
        seen.add(pattern)
        lines.append(f"{pattern} text eol=lf")
    lines.append(GITATTRIBUTES_END)
    return "\n".join(lines) + "\n"


def workspace_content_root(instance_path: Path) -> str | None:
    """This instance's private-workspace content root, when it is itself a
    workspace. Read from .eif/workspace.yaml so the managed block keeps the
    line across upgrades instead of it being a one-time scaffold decision."""
    config_path = instance_path / ".eif" / "workspace.yaml"
    ...
    if not isinstance(data, dict):
        return None
    root = (data.get("content") or {}).get("root")
...
def main(argv: list[str] | None = None) -> int:
    ...
    existing_ga_text = ga_path.read_text(encoding="utf-8") if ga_path.exists() else None
    gitattributes_block = render_gitattributes_block(
        entrypoint,
        knowledge_index_path if knowledge_managed else None,
        workspace_content_root(instance_path),
    )
    try:
        ga_new_text, ga_action = render_merged_content(
            existing_ga_text, gitattributes_block, GITATTRIBUTES_MARKER, GITATTRIBUTES_END
        )
    except MarkerConflict as e:
        print(f"eif-init: .gitattributes has malformed EIF markers, refusing to write anything: {e}", file=sys.stderr)
        return 1
    ...
    # Idempotence: a re-run that resolves to exactly the same provenance must
    # not rewrite the lock just to move its clock forward. Without this, every
    # `eifctl upgrade` (and so every `eifctl projects upgrade --apply` across a
    # fleet) leaves each project dirty with a one-line timestamp diff that has
    # no content behind it, and that diff then blocks the next fleet run on its
    # own clean-tree gate. generated_at answers "when was this provenance
    # established", not "when did a command last run", so preserving it when
    # nothing else moved is also the more truthful record.
    existing_lock_data = None
    if lock_path.exists():
        try:
            existing_lock_data = yaml.safe_load(lock_path.read_text(encoding="utf-8"))
        except (OSError, yaml.YAMLError):
            existing_lock_data = None
    if isinstance(existing_lock_data, dict) and "generated_at" in existing_lock_data:
        candidate = dict(lock_data)
        previous = dict(existing_lock_data)
        candidate.pop("generated_at", None)
        previous_timestamp = previous.pop("generated_at", None)
        if candidate == previous and isinstance(previous_timestamp, str):
            lock_data["generated_at"] = previous_timestamp
    ...
    try:
        ...
        # index: this file exists to pin LF, so it must not be written through
        # the platform's own newline translation.
        ga_next = ga_path.with_name(ga_path.name + ".next")
        staged_next_paths.append(ga_next)
        ga_bytes = ga_new_text.encode("utf-8")
        _maybe_fault_before("gitattributes")
        _maybe_fault_partial("gitattributes", ga_next, ga_bytes)
        ga_next.write_bytes(ga_bytes)
        stages.append(_Stage("gitattributes", ga_next, ga_path, is_dir=False))
