...
- Local Codex CLI `0.144.5` help підтвердив `codex resume [SESSION_ID]
  [PROMPT]`, `--last` і `-C/--cd`. Resume не запускав model call.
- Current official [app commands](https://developers.openai.com/codex/app/commands)
  документують `codex://threads/new` з encoded `prompt` і absolute `path`.
  Link заповнює composer, але не надсилає prompt.
- Owner-approved local deep-link canary повернув exit `0`, а ChatGPT desktop
  process був присутній. Доступна window metadata не довела, що відкрилися
  саме потрібний chat, project і composer. Тому `open_new_chat` лишається
  `manual` з `evidence_status: inconclusive`, а не `auto_open_verified`.
- `eifctl session handoff` генерує escaped candidate link як manual-only
  output. `--open` fail-closed і нічого не відкриває автоматично.
- `auto_submit` є `unsupported`; користувач перевіряє destination та
  підтверджує send вручну.
