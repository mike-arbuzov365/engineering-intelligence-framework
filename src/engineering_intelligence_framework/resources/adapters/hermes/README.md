...
- Local Hermes Agent `0.20.0` help підтвердив `--continue`, `--resume` і
  `--in`. Current official
  [slash-command reference](https://hermes-agent.nousresearch.com/docs/reference/slash-commands)
  також документує `/new`, `/resume`, `/branch` і `/handoff`.
- Current installed `session.create` handler пройшов disposable structural
  canary: створено process-local session record, model calls `0`, persistent
  config writes `0`. Перший canary run виявив відсутній test stub і був
  виправлений перед PASS.
- Canary не довів visible Desktop chat, правильний profile attachment або
  послідовний control transfer. Тому automatic Desktop handoff не активовано.
- Fallback: `/new <logical-session-id>`, manual handoff prompt і
  `resume-audit`; `session.create` не викликається EIF автоматично.
- `pre_compact` і `post_compact` не верифіковані end-to-end. `/compress` не
  замінює canonical checkpoint.
